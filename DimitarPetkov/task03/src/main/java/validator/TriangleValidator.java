...
import static common.ConstantMessages.SIDE_NOT_VALID_NUMBER;
import static common.ConstantMessages.TRIANGLE_NOT_VALID;
        ...
        for (int i = 1; i <= 3; i++) {
            ...
            while (!isValidSide) {
                if (isValidNumber(input[i])) {
                    ...
                    System.out.printf(SIDE_NOT_VALID_NUMBER, i);
        ...
        boolean isValidTriangle = false;
            while (!isValidTriangle) {
                ...
                if (sidesArray[i] >= referencePerimeter) {
                    System.out.println(TRIANGLE_NOT_VALID);
                    input = requestInput();
